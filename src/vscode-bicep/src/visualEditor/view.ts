...
export class BicepVisualEditorView extends Disposable {
  public static viewType = "bicep.visualEditor";
  ...
  public static create(
    ...
    const visualizerTitle = `Visual Editor ${path.basename(documentUri.fsPath)}`;
    ...
    return new BicepVisualEditorView(
      languageClient,
      webviewPanel,
      extensionUri,
      documentUri
  ...
  private handleDidReceiveMessage(message: Message): void {
    switch (message.kind) {
      case "READY":
        getLogger().debug(
          `Visual Editor for ${this.documentUri.fsPath} is ready.`
        );
  ...
  private createWebviewHtml() {
    ...
    const scriptUri = this.webviewPanel.webview.asWebviewUri(
      vscode.Uri.joinPath(this.extensionUri, "out", "visualEditor.js")
    );
